...
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    let segment_frame_bytes = env_usize(
        "SOVRIGHT_RELAY_RAW_SEGMENT_FRAME_BYTES",
        DEFAULT_SEGMENT_FRAME_BYTES,
    )?;
    let packet_delay =
        Duration::from_micros(env_u64("SOVRIGHT_RELAY_PROBE_PACKET_DELAY_MICROS", 0)?);
    let segment_delay = Duration::from_millis(env_u64("SOVRIGHT_RELAY_PROBE_SEGMENT_DELAY_MS", 0)?);
    ...
    println!(
        "{{\"event\":\"sovright_relay_raw_segment_probe_sent\",\"authenticated\":true,\"relays\":{},\"segments\":{},\"packets_sent\":{},\"block_hash\":\"{}\",\"raw_block_bytes\":{},\"segment_frame_bytes\":{}}}",
        relay_addrs.len(),
        segments.len(),
        packets_sent,
...
fn raw_block_from_env() -> Result<Vec<u8>, Box<dyn std::error::Error + Send + Sync>> {
    if let Ok(path) = env::var("SOVRIGHT_RELAY_RAW_BLOCK_HEX_FILE") {
        return decode_hex_block(&fs::read_to_string(path)?);
    }
    let value = env::var("SOVRIGHT_RELAY_RAW_BLOCK_HEX")?;
...
fn authenticate_raw_segment_chunk(session: &RelaySession, chunk: &sovright_relay::Chunk) -> Chunk {
...
fn raw_block_header_hash(header: &[u8]) -> [u8; 32] {
    sovright_relay::zcash_block_hash(header)
}

#[cfg(test)]
mod tests {
    use super::*;
    use sovright_relay::RawBlockSegment;
    ...
    fn authenticated_raw_segment_chunk_preserves_segment_message_type() {
        ...
        assert_eq!(
            ...
            sovright_relay::MessageType::RawBlockSegment
        );
