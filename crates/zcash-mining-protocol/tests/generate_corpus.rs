...
fn generate_fuzz_corpus() {
    ...
    fs::write(
        format!("{}/valid_job", dir),
        encode_new_equihash_job(&job).unwrap(),
    )
    .unwrap();
    ...
    fs::write(
        format!("{}/accepted", dir),
        encode_submit_shares_response(&accepted).unwrap(),
    )
    .unwrap();
    ...
    fs::write(
        format!("{}/rejected_stale", dir),
        encode_submit_shares_response(&rejected).unwrap(),
    )
    .unwrap();
    ...
    fs::write(
        format!("{}/rejected_other", dir),
        encode_submit_shares_response(&other).unwrap(),
    )
    .unwrap();
    ...
    fs::write(
        format!("{}/valid_target", dir),
        encode_set_target(&target).unwrap(),
    )
    .unwrap();
    ...
    for (src, dst) in [
        ...
        for entry in fs::read_dir(&src_dir).unwrap() {
            ...
            fs::copy(
                entry.path(),
                format!("{}/{}", dst_dir, entry.file_name().to_str().unwrap()),
            )
            .unwrap();
        }
    }
