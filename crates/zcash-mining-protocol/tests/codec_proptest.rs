...
use zcash_mining_protocol::codec::{
    Decodable, Encodable, decode_new_equihash_job, decode_set_target, decode_submit_share,
    decode_submit_shares_response, encode_new_equihash_job, encode_set_target, encode_submit_share,
    encode_submit_shares_response,
};
use zcash_mining_protocol::messages::{
    NewEquihashJob, RejectReason, SetTarget, ShareResult, SubmitEquihashShare, SubmitSharesResponse,
};
...
fn arb_submit_equihash_share() -> impl Strategy<Value = SubmitEquihashShare> {
    (
        ...
        .prop_map(
            |(channel_id, sequence_number, job_id, nonce_2, time, sol_vec)| {
...
fn arb_submit_shares_response() -> impl Strategy<Value = SubmitSharesResponse> {
    (
        ...
        .prop_map(
            |(channel_id, sequence_number, result)| SubmitSharesResponse {
                channel_id,
                sequence_number,
                result,
            },
...
fn arb_set_target() -> impl Strategy<Value = SetTarget> {
    (any::<u32>(), any::<[u8; 32]>())
        .prop_map(|(channel_id, target)| SetTarget { channel_id, target })
}
