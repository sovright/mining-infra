...
fn generate_fuzz_corpus() {
    ...
    fs::write(
        format!("{}/valid", dir),
        encode_allocate_token(&msg).unwrap(),
    )
    .unwrap();
    ...
    fs::write(
        format!("{}/valid", dir),
        encode_allocate_token_success(&msg).unwrap(),
    )
    .unwrap();
    ...
    fs::write(
        format!("{}/valid", dir),
        encode_set_custom_job(&msg).unwrap(),
    )
    .unwrap();
    ...
    fs::create_dir_all(&dir).unwrap();
    let msg =
        SetCustomMiningJobError::new(1, 1, SetCustomMiningJobErrorCode::InvalidToken, "bad token");
    fs::write(
        format!("{}/valid", dir),
        encode_set_custom_job_error(&msg).unwrap(),
    )
    .unwrap();
    ...
    fs::write(
        format!("{}/valid", dir),
        encode_push_solution(&msg).unwrap(),
    )
    .unwrap();
    ...
    let msg = SetFullTemplateJob {
        ...
        time: 1700000000,
        bits: 0x1d00ffff,
        tx_short_ids: vec![[0xdd; 32]],
        tx_data: vec![vec![0x01; 50]],
    };
    fs::write(
        format!("{}/valid", dir),
        encode_set_full_template_job(&msg).unwrap(),
    )
    .unwrap();
    ...
    fs::write(
        format!("{}/valid", dir),
        encode_set_full_template_job_success(&msg).unwrap(),
    )
    .unwrap();
    ...
    fs::create_dir_all(&dir).unwrap();
    let msg =
        SetFullTemplateJobError::new(1, 1, SetFullTemplateJobErrorCode::InvalidToken, "bad token");
    fs::write(
        format!("{}/valid", dir),
        encode_set_full_template_job_error(&msg).unwrap(),
    )
    .unwrap();
    ...
    fs::write(
        format!("{}/valid", dir),
        encode_get_missing_transactions(&msg).unwrap(),
    )
    .unwrap();
    ...
    fs::write(
        format!("{}/valid", dir),
        encode_provide_missing_transactions(&msg).unwrap(),
    )
    .unwrap();
